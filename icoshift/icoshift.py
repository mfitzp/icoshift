...
def icoshift(xT,  xP,  inter='whole',  n='f',  options=[1,  1,  0,  0,  0],  Scal=None):
    ...
    if max(Scal.shape) != xP.shape[1]:
        error('X and Scal are not of compatible length %d vs. %d' % ( max(Scal.shape), xP.shape[1] ) )
    ...
    if xT=='average':
        ...
        if xT=='median':
            ...
            if xT=='average2':
                ...
                avg2_flag=1
                if options[0] != 0:
                    avg_power=raw_input('which multiplier for the 2nd average? [default is 3] ')
                else:
                    avg_power=3
    ...
    if max(options.shape) < max(options_co.shape):
        options[(end + 1-1):max(options_co.shape)]=options_co[(max(options.shape) + 1-1):]
    ...
    if flag_coshift:
        ...
        xPo=xP
        xP,nil,wint = icoshift(xT,xP,'whole',n_co,np.array([0,1,0]).reshape(1,-1))
    ...
    if type(inter) == str:
        if inter=='whole':
            ...
            if not any(inter == '-'):
                ...
                interv=sort(scal2pts(str2double(cat(0,interv[:])),Scal,prec))
                if interv.size != 2:
        ...
        if is_number(inter):
            if inter.size == 1:
                if fix(inter) == inter:
                    ...
                    remain=mod(mP,inter)
                    N=fix(mP / inter)
                    startint=np.array([(range(1,((remain - 1) * (N + 1) + 1+1),(N + 1))).T,(range((remain - 1) * (N + 1) + 1 + 1 + N,(mP+1),N)).T]).reshape(1,-1)
                ...
                flag2=np.array_equal(fix(inter),inter) and max(inter.shape) > 1 and np.array_equal(np.array([1,np.max(inter) - np.min(inter) + 1]).reshape(1,-1),inter.shape) and np.array_equal(unique(np.diff(inter,1,2)),1)
    ...
    if type(n)==str and (len(n) > 1 or n not in ['b','f']):
        ...
        if is_number(n):
            ...
            if n.size > 1:
                wmsg=sprintf('"n" must be a scalar/character; first element (i.e. %i) used',round_(n))
            if scfl and n != fix(n):
                wmsg=sprintf('"n" must be an integer if Scal is ignorde; first element (i.e. %i) used',round_(n))
            else:
                if options[4]:
    ...
    if vec(flag).any():
        ...
        if flag2:
            intern_=RemoveNaN(np.array([0, pma - pmi]).reshape(1,-1),cat(0,xT[:,inter],xP[:,inter]),Select)
            if intern_.shape[0] != 1:
                error('Reference region contains a pattern of missing that cannot be handled consistently')
            else:
                if not  np.array_equal(intern_,np.array([1,inter[-2] - inter[0] + 1]).reshape(1,-1)):
                    warning('iCoShift:miss_refreg','The missing values at the boundaries of the reference region will be ignored')
            intern_=range(inter[0] + intern_[0],(inter[0] + intern_[1]+1))
        else:  
            intern_,flag_nan = RemoveNaN(Ref(inter),cat(0,xT,xP),Select, flags=True)
        ...
        if max(intern_.shape) != max(inter.shape) and not  flag2:
            ...
            if whole:
                ...
                warning('iCoShift:Missing_values','\nTo handle the pattern of missing values, %i segments are created/removed',abs(max(intern_.shape) - max(inter.shape)) / 2)
                inter=intern_
                nint,mint = inter.shape
    xCS=xP
    mi = np.min(inter)
    ...
    flag=max(inter.shape) > 1 and np.array_equal(np.array([1,pma - pmi + 1]).reshape(1,-1),inter.shape) and np.array_equal(unique(np.diff(inter,1,2)),1)

    if flag:
        if options[0]:
            if n=='b':
                print('Automatic searching for the best "n" for the reference window "refW" enabled \That can take a longer time \\n')
            else:
                if n=='f':
                    print('Fast automatic searching for the best "n" for the reference window "refW" enabled \\n')
        if max_flag:
            np.max(np.sum(xP))
        ...
        if options[0]:
            if n=='b':
                print('Automatic searching for the best "n" for each interval enabled \That can take a longer time...')
                
            else:
                if n=='f':
                    print('Fast automatic searching for the best "n" for each interval enabled')
        for i in range(0,allint.shape[0]):
            if options[0] != 0:
                if whole:
                    print('Co-shifting the whole %s samples...' % nP)
                else:
                    print('Co-shifting interval no. %s of %s...' % (i,allint.shape[0]) )

            # xP(:,allint(i,2):allint(i,3));
            intervalnow=xP[:,allint[i,1]:allint[i,2]] #FIXME? 0:2, or 1:2?
            ...
            for i in range(0, allint.shape[0]):
                if options[0] != 0:
                    if whole:
                        ...
                        print('Co-shifting again interval no. %g of %g... ',i,allint.shape[0])
                        
                intervalnow=xP[:,allint[i,1]:allint[i,2]]
                target1=mean(xCS[:,allint[i,1]:allint[i,2]])
    ...
    if frag:
        ...
        for i_sam in range(0,nP):
            for i_seg in range(0,InOr.shape[0]):
                Xn[i_sam,InOr[i_seg,0]:InOr[i_seg,1]+1]=xCS[i_sam,InOr[i_seg,2]:InOr[i_seg,3]+1]
                if loc_ind[i_sam] < 0:
                    if flag_nan[i_seg,0,i_sam]:
                        Xn[i_sam,InOr[i_seg,0]:InOr[i_seg,0] - loc_ind[i_sam,0]+1]=np.nan
                else:
                    if loc_ind[i_sam] > 0:
...
def coshifta(xT,xP,refW=np.array([0]),n=np.array([1,2,3]),options=np.array([])):
    ...
    if (mT != mP):
        error('Target "xT" and sample "xP" must be of compatible size (same vectors, same matrices or row + matrix of rows)')
    if np.any(n <= 0):
    ...
    if n=='b':
        auto=1
        if rw != 1:
            n=np.max(fix(0.05 * mR),10)
            src_step=fix(mR * 0.05)
        else:
            n=np.max(fix(0.05 * mP),10)
            src_step=fix(mP * 0.05)
        try_last=0
    else:
        if n=='f':
            auto=1
    ...
    if auto == 1:
        while auto == 1:
            ...
            if Filling == - np.inf:
                xtemp = cat( 1, np.tile( xP[:,:1], (1,n) ), xP, np.tile( xP[:,-1:,], (1,n) ) )
                #xtemp=np.array([xP[:,np.ones(1,n)],xP,xP[:,(mP[0,np.ones(1,n)]-1)]]) #.reshape(1,-1)
            ...
            for i_block in range(0, nBlocks):
                block_indices=range(indBlocks[0,i_block], indBlocks[0,i_block + 1])
            ...
            if (np.max(abs(ind)) == n) and try_last != 1:
                ...
                if (np.max(abs(ind)) < n) and n + src_step < len(refW) and try_last != 1:
                    ...
                    auto=0
                    if options[0] != 0:
                        print('Best shift allowed for this interval = %g \\n',n)
    else:
        if Filling == - np.inf:
            xtemp=np.array([xP[:, np.ones((1,n))],xP,xP[:, mP[0,np.ones((1,n))] ]]).reshape(1,-1)
...
def defints(xP,interv,opt):
    ...
    if opt[0] == 2 and (sizechk != 0):
        ...
        if opt[0] != 0 and (sizechk != 0):
            print(' Warning: the last interval will not fulfill the selected intervals size "inter"=%g. \ Size of the last interval = %g ',interv,plus)
...
def CC_FFTShift(T, X=False, options=np.array([])):
    ...
    optionsDefault=np.array([-np.fix(dimT[-1] * 0.5), np.fix(dimT[-1] * 0.5), len(T.shape)-1,1,np.nan]) #.reshape(1,-1)
    ...
    if flag_miss:
        ...
        for i_signal in range(0, mP):
            ...
            Limits=RemoveNaN(np.array([0,nP-1]).reshape(1,-1),X_fft[:,i_signal].T,np.all)
            if not  np.array_equal(Limits.shape,np.array([1,2]).reshape(1,-1)):
                error('Missing values can be handled only if leading or trailing')
            ...
            if MissOff[i_signal] > 1:
                X_fft[0:Limits[1] - Limits[0] + 1,i_signal]=X_fft[Limits[0]:Limits[1],i_signal]
            if Limits[1] < nP:
    ...
    if options[0] < 0 and options[1] > 0:
        ...
        ind = range( int(len_fft+options[0]), int(len_fft) ) + range( 0,  int(options[1]+1) )
        ...
        #ind = [len_fft + Options(1) + 1:len_fft + Options(2),1];
        ind= range( int(len_fft + options[0]), int(len_fft + options[1]+1) ) + [1]
    ...
    T_fft=np.tile(T_fft,(1,dimX[0])) #np.reshape(T_fft[:,:,np.ones( (dimX[0],1), dtype=bool )],X_fft.shape)
    dT = X_fft * T_fft
    cc=np.fft.ifft(dT,len_fft,axis=0)

    # reshape(cc(ind,:),[Options(2) - Options(1) + 1,prod(dimX(ord(2:end - 1))),dimX(1)]);
    # np.array([options[1] - options[0] + 1, np.prod(dimX[ord_[1: - 1]]), dimX[0]]))
    if len(ord_[1:-1])==0:
        k = 1
    else:
        ...
        if Shift[i_X] >= 0:
            ...
            if options[4] == - np.inf:
                ...
                if len(o)>0:
                    ...
                    indw=cat(1,
                            indw,
                            np.arange(dimX[TimeDim] - Shift[i_X], dimX[TimeDim]).reshape(1,-1)
                            )
        else:
            if Shift[i_X] < 0:
                ...
                if options[4] == - np.inf:
                    ...
                    indw = cat(1, np.arange(0,-Shift[i_X]).reshape(1,-1) , indw);
...
def RemoveNaN(B, Signal, Select=np.any, flags=False):
    ...
    if flags:
        flag=np.empty( (C.shape[0],2,Signal.shape[0]), dtype=bool ) #.reshape(1,-1)
...
def ExtractSegments(X,Segments):
    ...
    flag_ni=np.any(np.diff(Segments[:,0]) < 0) or np.any(np.diff(Segments[:,1]) < 0)
    flag_ab=np.any(Sd < 2)
